...
const themes = {
    dark: {
        ...
        infoBg: 'rgba(99,102,241,0.1)', infoBorder: 'rgba(99,102,241,0.25)', infoText: '#a5b4fc',
    },
    light: {
        ...
        infoBg: 'rgba(99,102,241,0.07)', infoBorder: 'rgba(99,102,241,0.2)', infoText: '#4f46e5',
    }
};
        ...
        <div style={{ position: 'fixed', inset: 0, background: t.overlayBg, backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1500 }} onClick={onClose}>
            <div style={{ background: t.modalBg, border: `1px solid ${t.border}`, borderRadius: '20px', width: '90%', maxWidth: '560px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 32px 80px rgba(0,0,0,0.5)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ padding: '16px 24px', borderTop: `1px solid ${t.border}`, display: 'flex', gap: '10px' }}>
                    <button onClick={() => handleSubmit(true)} disabled={loading} style={{ flex: 1, padding: '10px', background: t.inputBg, border: `1px solid ${t.border}`, borderRadius: '9px', color: t.textSecondary, fontSize: '13px', fontWeight: '500', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                        <Edit2 size={13} /> Save Draft
                    </button>
                    <button onClick={() => handleSubmit(false)} disabled={loading} style={{ flex: 2, padding: '10px', background: loading ? t.inputBg : 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: 'none', borderRadius: '9px', color: loading ? t.textMuted : '#fff', fontSize: '13px', fontWeight: '600', cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px', opacity: loading ? 0.7 : 1, boxShadow: loading ? 'none' : '0 3px 12px rgba(99,102,241,0.35)' }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: t.textMuted }}>
                            <div style={{ width: 22, height: 22, borderRadius: '50%', overflow: 'hidden', background: t.accentBg, border: `1px solid ${t.accentBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: t.accentText, flexShrink: 0 }}>
                                {avatarSrc ? <img src={avatarSrc} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (report.author_name || '?').charAt(0).toUpperCase()}
                            </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
                    ...
                    {canReview && report.status === 'pending' && (
                        <div style={{ background: t.surfaceCard, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '16px' }}>
                            ...
                            <textarea value={feedback} onChange={e => setFeedback(e.target.value)} placeholder="Add a comment for the submitter…" rows={3} style={{ ...inputSt(t), resize: 'vertical', marginBottom: '12px' }} />
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <button onClick={() => doAction('reject')} disabled={loading} style={{ flex: 1, padding: '9px', background: t.dangerBg, border: `1px solid ${t.dangerBorder}`, borderRadius: '9px', color: t.dangerText, fontSize: '13px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                                    <X size={14} /> Reject
                                </button>
                                <button onClick={() => doAction('approve')} disabled={loading} style={{ flex: 2, padding: '9px', background: '#10b981', border: 'none', borderRadius: '9px', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', boxShadow: '0 3px 10px rgba(16,185,129,0.3)' }}>
        ...
        <div style={{ background: t.surfacePrimary, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '16px 18px', transition: 'border-color 0.15s', cursor: 'pointer' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginLeft: '12px' }}>
                    ...
                    {canDelete && (
                        <button onClick={e => { e.stopPropagation(); onDelete(report); }} style={{ padding: '5px', background: 'none', border: 'none', color: t.dangerText, cursor: 'pointer', display: 'flex', borderRadius: '6px' }}
...
const ReportManagement = ({ dark = true }) => {
    ...
    const fetchReports = useCallback(async () => {
        ...
        try {
            const [rRes, tRes] = await Promise.all([
                fetch(`${API_BASE}/task-reports`, { headers: await getAuthHeaders() }),
    ...
    const handleSubmit = async (data) => {
        try {
            const res  = await fetch(`${API_BASE}/task-reports`, { method: 'POST', headers: await getAuthHeaders(), body: JSON.stringify(data) });
    ...
    const handleApprove = async (id, feedback) => {
        try {
            const res = await fetch(`${API_BASE}/task-reports/${id}/approve`, { method: 'PATCH', headers: await getAuthHeaders(), body: JSON.stringify({ feedback }) });
    ...
    const handleReject = async (id, feedback) => {
        try {
            const res = await fetch(`${API_BASE}/task-reports/${id}/reject`, { method: 'PATCH', headers: await getAuthHeaders(), body: JSON.stringify({ feedback }) });
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/task-reports/${deleteReport.id}`, { method: 'DELETE', headers: await getAuthHeaders() });
    ...
    return (
        <div style={{ padding: '24px 28px', maxWidth: '1200px', margin: '0 auto', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                <div>
                    ...
                    <p style={{ margin: '5px 0 0', fontSize: '13px', color: t.textMuted }}>
                        {canReview ? 'View, approve and give feedback on reports submitted by your team' : 'Submit reports on your work — visible to your company admins'}
                    </p>
                </div>
                ...
                <div style={{ padding: '14px 18px', borderBottom: `1px solid ${t.border}`, display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', gap: '2px', background: t.inputBg, borderRadius: '9px', padding: '3px' }}>
                        {[
                            ...
                            <button key={tb.id} onClick={() => setTab(tb.id)} style={{ padding: '6px 13px', background: tab === tb.id ? t.surfacePrimary : 'none', border: tab === tb.id ? `1px solid ${t.border}` : '1px solid transparent', borderRadius: '7px', color: tab === tb.id ? t.textPrimary : t.textMuted, fontSize: '12px', fontWeight: tab === tb.id ? '600' : '400', cursor: 'pointer', transition: 'all 0.15s', whiteSpace: 'nowrap' }}>
                                {tb.label}
                            </button>
                        ))}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 12px', background: t.inputBg, border: `1px solid ${t.border}`, borderRadius: '9px', flex: 1, minWidth: '180px', maxWidth: '280px', flexBasis: 'min(100%, 280px)' }}>
                        ...
                        <input type="text" placeholder="Search reports…" value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, background: 'none', border: 'none', color: t.textPrimary, fontSize: '13px', outline: 'none' }} />
                    ...
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ padding: '7px 12px', background: t.selectBg, border: `1px solid ${t.border}`, borderRadius: '9px', color: t.selectText, fontSize: '12px', cursor: 'pointer', outline: 'none' }}>
            ...
            {viewReport && <ReportDetailModal t={t} report={viewReport} onClose={() => setViewReport(null)} onApprove={handleApprove} onReject={handleReject} canReview={canReview} />}
            {deleteReport && (
