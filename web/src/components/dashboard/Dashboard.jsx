...
const PERSONAL = {
    ...
    accentGrad:'linear-gradient(135deg,#7c3aed 0%,#5b21b6 100%)',headerGrad:'linear-gradient(135deg,#7c3aed,#a78bfa)',
    ...
    inputBg:'rgba(255,255,255,0.04)',selectBg:'#0f1628',modalBg:'#080d1a',overlay:'rgba(0,0,0,0.8)',
    sidebarBg:'#080d1a',sidebarBorder:'rgba(255,255,255,0.05)',sidebarActive:'rgba(124,58,237,0.14)',sidebarActiveBorder:'rgba(124,58,237,0.4)',
    online:'#10b981',onlineBg:'rgba(16,185,129,0.12)',offline:'#ef4444',offlineBg:'rgba(239,68,68,0.12)',
    modeLabel:'Personal',modeIcon:'👤',
};
const COMPANY = { ...PERSONAL,accent:'#0ea5e9',accentLight:'#38bdf8',accentBg:'rgba(14,165,233,0.1)',accentBorder:'rgba(14,165,233,0.25)',accentGrad:'linear-gradient(135deg,#0ea5e9 0%,#0284c7 100%)',headerGrad:'linear-gradient(135deg,#0ea5e9,#38bdf8)',sidebarActive:'rgba(14,165,233,0.12)',sidebarActiveBorder:'rgba(14,165,233,0.4)',modeLabel:'Company',modeIcon:'🏢' };
const LIGHT_PERSONAL = {
    bg:'#f5f3ff',surface:'#ffffff',surfaceRaised:'#faf9ff',card:'rgba(0,0,0,0.02)',cardHover:'rgba(0,0,0,0.04)',
    ...
    sidebarBg:'#ffffff',sidebarBorder:'rgba(0,0,0,0.06)',sidebarActive:'rgba(124,58,237,0.07)',sidebarActiveBorder:'rgba(124,58,237,0.3)',
    online:'#059669',onlineBg:'rgba(5,150,105,0.09)',offline:'#dc2626',offlineBg:'rgba(220,38,38,0.09)',
    modeLabel:'Personal',modeIcon:'👤',
};
const LIGHT_COMPANY = { ...LIGHT_PERSONAL,bg:'#f0f9ff',accentGrad:'linear-gradient(135deg,#0ea5e9 0%,#0284c7 100%)',headerGrad:'linear-gradient(135deg,#0ea5e9,#38bdf8)',accent:'#0ea5e9',accentLight:'#0284c7',accentBg:'rgba(14,165,233,0.06)',accentBorder:'rgba(14,165,233,0.18)',sidebarActive:'rgba(14,165,233,0.07)',sidebarActiveBorder:'rgba(14,165,233,0.3)',modeLabel:'Company',modeIcon:'🏢' };
...
const TaskModal = ({ t, title, initialData, onClose, onSave, isOnline }) => {
    ...
    const handleSubmit=async(e)=>{ e.preventDefault(); if(!formTitle.trim()){setError('Title is required');return;} setLoading(true);setError(''); const err=await onSave({title:formTitle.trim(),description,priority,deadline:deadline||null}); if(err){setError(err);setLoading(false);} };
    return (
        <div style={{ position:'fixed',inset:0,background:t.overlay,backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'16px' }} onClick={onClose}>
            <div style={{ background:t.modalBg,border:`1px solid ${t.borderMid}`,borderRadius:'20px',padding:'28px',width:'100%',maxWidth:'480px',boxShadow:'0 40px 80px rgba(0,0,0,0.5)' }} onClick={e=>e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} style={{ display:'flex',flexDirection:'column',gap:'16px' }}>
                    ...
                    <div><label style={{ display:'block',fontSize:'11px',fontWeight:'600',color:t.textMuted,marginBottom:'5px',letterSpacing:'0.06em',textTransform:'uppercase' }}>Description</label><textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="What needs to be done?" style={{ width:'100%',minHeight:'80px',padding:'10px 12px',background:t.inputBg,border:`1px solid ${t.border}`,borderRadius:'8px',fontSize:'13px',color:t.textPrimary,resize:'vertical',boxSizing:'border-box',fontFamily:'inherit',outline:'none' }}/></div>
                    <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px' }}>
                        <div><label style={{ display:'block',fontSize:'11px',fontWeight:'600',color:t.textMuted,marginBottom:'5px',letterSpacing:'0.06em',textTransform:'uppercase' }}>Priority</label><select value={priority} onChange={e=>setPriority(e.target.value)} style={{ width:'100%',padding:'10px 12px',background:t.selectBg,border:`1px solid ${t.border}`,borderRadius:'8px',fontSize:'13px',color:t.textPrimary,fontFamily:'inherit',outline:'none',cursor:'pointer' }}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="urgent">Urgent</option></select></div>
...
const DeleteModal = ({ t, task, onConfirm, onCancel, loading }) => (
    <div style={{ position:'fixed',inset:0,background:t.overlay,backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000,padding:'16px' }} onClick={()=>!loading&&onCancel()}>
        <div style={{ background:t.modalBg,border:`1px solid ${t.dangerBorder}`,borderRadius:'20px',padding:'28px',width:'100%',maxWidth:'400px',boxShadow:'0 40px 80px rgba(0,0,0,0.5)' }} onClick={e=>e.stopPropagation()}>
            <div style={{ display:'flex',alignItems:'center',gap:'14px',marginBottom:'16px' }}>
                <div style={{ width:'46px',height:'46px',borderRadius:'13px',background:t.dangerBg,border:`1px solid ${t.dangerBorder}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}><Trash2 size={20} color={t.danger}/></div>
                <div><h3 style={{ margin:0,fontSize:'16px',fontWeight:'700',color:t.textPrimary }}>Delete Task</h3><p style={{ margin:'2px 0 0',fontSize:'11px',color:t.textMuted }}>This cannot be undone</p></div>
            </div>
            <p style={{ fontSize:'13px',color:t.text,margin:'0 0 22px',lineHeight:1.6 }}>Are you sure you want to permanently delete <strong style={{color:t.textPrimary}}>"{task.title}"</strong>?</p>
            <div style={{ display:'flex',gap:'10px',justifyContent:'flex-end' }}>
                <Btn t={t} variant="ghost" onClick={onCancel} disabled={loading}>Cancel</Btn>
                <Btn t={t} variant="danger" onClick={onConfirm} disabled={loading} style={{ background:t.danger,color:'#fff',border:'none',boxShadow:`0 4px 14px ${t.danger}40` }}><Trash2 size={13}/>{loading?'Deleting…':'Delete'}</Btn>
            </div>
        </div>
    </div>
);
                ...
                <div style={{ padding:'22px 24px',overflowY:'auto',flex:1 }}>
                    ...
                    {tab==='profile'&&(
                        <div style={{ display:'flex',flexDirection:'column',gap:'18px' }}>
                            ...
                            <div style={{display:'flex',justifyContent:'flex-end'}}><Btn t={t} variant="primary" onClick={saveProfile} disabled={loading||!isOnline}><Save size={13}/>{loading?'Saving…':'Save Changes'}</Btn></div>
                        </div>
                    )}
                    {tab==='security'&&(
                        <div style={{ display:'flex',flexDirection:'column',gap:'16px' }}>
                            ...
                            {[['Current Password',currentPw,setCurrentPw,showCurrent,setShowCurrent],['New Password',newPw,setNewPw,showNew,setShowNew]].map(([lbl,val,setter,show,setShow],idx)=>(
                                <div key={lbl}><label style={{display:'block',fontSize:'11px',fontWeight:'600',color:t.textMuted,marginBottom:'5px',letterSpacing:'0.06em',textTransform:'uppercase'}}>{lbl}</label><div style={{position:'relative'}}><input type={show?'text':'password'} value={val} onChange={e=>setter(e.target.value)} placeholder={idx===0?'Current password':'Min. 8 characters'} style={{width:'100%',padding:'10px 40px 10px 12px',background:t.inputBg,border:`1px solid ${t.border}`,borderRadius:'8px',fontSize:'13px',color:t.textPrimary,boxSizing:'border-box',fontFamily:'inherit',outline:'none'}}/><button type="button" onClick={()=>setShow(!show)} style={{position:'absolute',right:'10px',top:'50%',transform:'translateY(-50%)',background:'none',border:'none',color:t.textMuted,cursor:'pointer',display:'flex'}}>{show?<EyeOff size={14}/>:<Eye size={14}/>}</button></div>{idx===1&&newPw&&(<div style={{marginTop:'8px'}}><div style={{display:'flex',gap:'3px',marginBottom:'4px'}}>{[1,2,3,4].map(i=><div key={i} style={{flex:1,height:'3px',borderRadius:'2px',background:i<=pwStrength?pwColors[pwStrength]:t.border,transition:'background 0.2s'}}/>)}</div><span style={{fontSize:'10px',color:pwColors[pwStrength],fontWeight:'600'}}>{['','Too weak','Weak','Good','Strong'][pwStrength]}</span></div>)}</div>
                            ))}
...
const LockedView = ({ t, label, onSetup }) => (
    <div style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'400px',gap:'14px',padding:'40px' }}>
        ...
        <div style={{textAlign:'center'}}><h3 style={{margin:'0 0 6px',fontSize:'17px',fontWeight:'700',color:t.textPrimary}}>{label}</h3><p style={{margin:0,fontSize:'13px',color:t.textMuted}}>This feature requires a company workspace.</p></div>
        <Btn t={t} variant="primary" onClick={onSetup}><Building2 size={14}/> Set Up Company</Btn>
    </div>
);
...
const Dashboard = () => {
    ...
    useEffect(()=>{
        ...
        fetch(`https://syncline1.onrender.com/api/company/team`, { headers:{ Authorization:`Bearer ${localStorage.getItem('accessToken')}` } })
            .then(r=>r.json())
